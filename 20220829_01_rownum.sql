...
● [rownum을 활용한 페이징]
   - rownum은 모조칼럼이므로 조건절에 직접 사용하지 말고, 실제칼럼으로 
     인식을 먼저 시킨 뒤 사용할 것을 추천
   - rownum칼럼은 셀프조인 후에 줄 번호를 추가하고 조건절에 활용한다
   
문) 줄번호(rownum)를 이용해서 줄번호 4행~6행을 조회
...
--3) 2)의 결과를 BB테이블로 만들고, 줄번호 4행~6행까지 조회하시오
