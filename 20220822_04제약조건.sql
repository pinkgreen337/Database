...
● [테이블 제약조건 에러 메세지]
-- primary key 제약조건
-- ORA-00001: 무결성 제약조건(SYSTEM.SYS_C007323)에 위배됩니다
-- sno 칼럼은 기본키(pk)이므로 중복된 값을 허용하지 않음
insert into sungjuk(sno, uname) values(1,'홍길동');        --2번 실행했을 때 에러 발생


-- not null 제약조건
-- ORA-0144 : NULL을 :("SYSTEM", "SUNGJUK", "UNAME")안에 삽입할 수 없습니다
...
-- check 제약조건
-- ORA-02290: 체크 제약조건(SYSTEM.SYS_C007321)이 위배되었습니다
...
-- addr 칼럼에 Incheon 허용 안함
insert into sungjuk(sno, uname, kor, eng, mat, addr)
values(1, '홍길동', 10, 20, 30, 'Incheon' );                -- addr 칼럼에 'Incheon' 허용 안함
